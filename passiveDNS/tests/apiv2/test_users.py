...
#/register post
    def test_register(self) -> None:
        response = client.post("/register", 
            json={"username":"TestUser2", "password":"user2", "token":self.user2Token})
    ...
    def test_register_username_taken(self) -> None:
        response = client.post("/register", 
            json={"username":"TestUser1", "password":"user1", "token":self.user2Token})
...
#/register/check post
    def test_token_check(self) -> None:
        response = client.post("/register/check", json={"token":self.user3Pending.token})
        self.assertEqual(response.status_code, 200)
...
#/request post
    def test_request(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["msg"], "request for access with mail user4@test.com sent to admin")
    ...
    def test_request_pending_exists(self) -> None:
        ...
        self.assertEqual(response.status_code, 500)
        data = response.json()
        self.assertEqual(data["detail"], "an invitation has already been sent to this email")
    
    def test_request_already_exists(self) -> None:
        response = client.post("/request", json={"email":"user4@test.com"})
        self.assertEqual(response.status_code, 500)
        data = response.json()
        self.assertEqual(data["detail"], "a request for this email has already been sent")

#/password put
    def test_password(self) -> None:
        response = client.put("/password", json={"current_password":"user1", "new_password":"user1changed"})
        self.assertEqual(response.status_code, 200)
    
    def test_password_invalid(self) -> None:
        response = client.put("/password", json={"current_password":"none", "new_password":"user1changed"})
        self.assertEqual(response.status_code, 401)
    
    def test_password_same(self) -> None:
        response = client.put("/password", json={"current_password":"user1changed", "new_password":"user1changed"})
        self.assertEqual(response.status_code, 400)
