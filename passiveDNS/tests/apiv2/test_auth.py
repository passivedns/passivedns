...
class AuthTest(unittest.TestCase):
    ...
    def test_login_admin(self) -> None:
        response = client.post("/token", json={"identity":"TestAdmin1", "password":"admin1"})
    ...
    def test_login_not_user(self) -> None:
        response = client.post("/token", json={"identity":"random", "password":"user"})
        self.assertEqual(response.status_code, 404)
    
    def test_login_wrong_password(self) -> None:
        response = client.post("/token", json={"identity":"TestUser1", "password":"random"})
