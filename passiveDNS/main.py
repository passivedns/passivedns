...
from apiv2.auth import auth_router, get_current_user, check_admin_role, check_admin_user_role, check_scheduler_role
...
api_router.include_router(domain_name_router, dependencies=[Depends(get_current_user), Depends(check_admin_user_role)])
api_router.include_router(resolution_router, dependencies=[Depends(get_current_user), Depends(check_admin_user_role)])
api_router.include_router(scheduler_router, dependencies=[Depends(get_current_user), Depends(check_scheduler_role)])
api_router.include_router(scheduler_admin_router, dependencies=[Depends(get_current_user), Depends(check_admin_role)])
